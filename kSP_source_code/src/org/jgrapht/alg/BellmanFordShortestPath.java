...
public class BellmanFordShortestPath<V, E> {
	 ...
	 * @throws Exception
	 */
	public String parseEdgeList(List<E> edgeList, V endVertex) throws Exception {
		String pathStr = "";

		V nextSource = startVertex;
		for (int i = 0; i < edgeList.size(); i++) {
			// find the source of tobecached path
			E edge = edgeList.get(i);
			// need to handle undirected edge in our case
			V sourceTmp = graph.getEdgeSource(edge);
			V sinkTmp = graph.getEdgeTarget(edge);
			if (((Integer) sourceTmp).compareTo((Integer) nextSource) != 0) {
				// ATTENTION: it is tricky for generic comparison of two Integer
				// numbers, here we know they are integers, therefore, we force
				// cast and have to use compareTo
				V tmp = sourceTmp;
				sourceTmp = sinkTmp;
				sinkTmp = tmp;
			}
			...
			if (((Integer) sourceTmp).compareTo((Integer) nextSource) != 0) {
				throw new Exception(nextSource + " " + sourceTmp + " " + sinkTmp + " " + startVertex + " "
						+ endVertex + " edge path wrong at " + i + "th " + edgeList);
			}
