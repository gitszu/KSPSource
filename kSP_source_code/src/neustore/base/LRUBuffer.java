...
public class LRUBuffer extends DBBuffer {
	...
	public int get(){
		DBBufferStoredElement s = (DBBufferStoredElement)buffer.get(0);
		HeapFilePage p = (HeapFilePage)s.object;
		return p.numRecs();
		...
		while ( i < buffer.size() ) {
			...
			if ( stored.file == file ) {
				...
				if ( stored.dirty ) {
					...
					//((HeapFilePage)stored.object).clear();
